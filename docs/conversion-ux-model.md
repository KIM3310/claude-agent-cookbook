# Conversion UX Model - claude-agent-cookbook
...
| Primary buyer or user | AI platform teams, product engineering leads, developer education teams, and model platform partners. |
| Value wedge | Teams adopting Claude need runnable examples, eval habits, and reusable implementation patterns. |
...
| First action | Open the README archived-status note and enterprise-readiness note, then inspect Recipes run with mocked tests only if this older domain proof is relevant. |
...
- Optional entry: Agent recipe workshop ($3k-$12k) when this archived proof exactly matches the buyer problem.
- Revival step: Implementation accelerator ($20k-$75k) only after the active successor and support boundary are clear.
- Longer-term path: Developer education bundle ($2k-$10k/month) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Workshop bookings only as a discussion metric until fresh validation exists.
